...
final class Utils {
	...
	public static String PICASSO_CACHE = "picasso-cache";
	...
	static File createDefaultCacheDir(Context context) {
		File cache = null;
		if(TextUtils.isEmpty(PicassoConfig.CACHEDIR))
			cache = new File(context.getApplicationContext().getCacheDir(), PICASSO_CACHE);
		else
			cache = new File(PicassoConfig.CACHEDIR);
		if (!cache.exists()) {
	...
	static long calculateDiskCacheSize(File dir) {
		if(PicassoConfig.CACHE_MAXSIZE > 0)
			return PicassoConfig.CACHE_MAXSIZE;
	...
	private static class OkHttpLoaderCreator {
		static Downloader create(Context context) {
			if(!TextUtils.isEmpty(PicassoConfig.CACHEDIR) && PicassoConfig.CACHE_MAXSIZE > 0){
				return new OkHttpDownloader(new File(PicassoConfig.CACHEDIR),PicassoConfig.CACHE_MAXSIZE);
			}else if(!TextUtils.isEmpty(PicassoConfig.CACHEDIR)) {
				return new OkHttpDownloader(new File(PicassoConfig.CACHEDIR));
			}
